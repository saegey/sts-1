    ...
    def enqueue(cls, athlete_id):
        sqs.send_message(
            ...
            MessageAttributes={
                "AthleteId": {
                    "DataType": "String",
                    "StringValue": str(athlete_id),
                }
            },
            MessageBody=(
                "recent-athlete-peak-{athlete_id}".format(
        ...
        print(
            "queuing recent peaks for {athlete_id}".format(
                athlete_id=athlete_id
            )
        )

    @classmethod
    def divide_chunks(cls, list_vals, n):
        for i in range(0, len(list_vals), n):
            yield list_vals[i : i + n]
