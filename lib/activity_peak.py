...
class ActivityPeak:
    ...
    def get_athlete_from_s3(cls, athlete_id):
        ...
        s3_response = s3_client.get_object(
            Key=peaks_filename, Bucket=config.strava_api_s3_bucket
        )
    ...
    def get_top(cls, athlete_id):
        ...
        print(
            "retrieved {num} rows from dynamo peaks table".format(
                num=len(items)
            )
        )
        peaks_organized = {}
        ...
        for key in peaks_organized.keys():
            peaks_organized[key].sort(key=lambda x: x["value"], reverse=True)
